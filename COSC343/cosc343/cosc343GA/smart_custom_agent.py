...
#────────── My Smart Custom Agent ──────────────────────────────────
def smart_custom_agent(state):
    pos = state["position"]
    if state["carrying"] == False:
        packages = state["packages"]

        others_robot = next(iter(state["other_robots"].values()))
        if not others_robot["carrying"]:
            others_pos = others_robot["position"]
            others_goal, their_distance = find_closest(others_pos, packages)
            my_distance = manhattan_distance(pos, others_goal)

            if their_distance < my_distance:
                packages.remove(others_goal)

        goal, distance = find_closest(pos, packages)
        if distance == 0:
            return PICKUP
        else:
